...
interface Game {
  key: string; name: string; color: string; icon: ReactNode
  best?: number; soon?: boolean; featured?: boolean
}
const GAMES: Game[] = [
  { key: 'daily', name: 'Daily Challenge', color: '#ffb43d', icon: <Star />, featured: true, best: 3820 },
  { key: 'pinpoint', name: 'PinPoint', color: '#3df5ff', icon: <Target />, best: 4820 },
  { key: 'slider', name: 'Slider', color: '#ffb43d', icon: <Slide />, best: 3910 },
  { key: 'gridfill', name: 'Grid-Fill', color: '#3dffa2', icon: <Grid />, soon: true },
  { key: 'matchup', name: 'Match-Up', color: '#ff4d8d', icon: <Link2 />, soon: true },
  { key: 'fitline', name: 'Fit-the-Line', color: '#a24bff', icon: <Chart />, soon: true },
]
...
function Chip({ color, icon, value }: { color: string; icon: ReactNode; value: string }) {
  return (
    <div className="flex items-center gap-2 px-3.5 py-2 rounded-full bg-white/5 border border-white/10 text-sm font-semibold"
      style={{ color }}>
...
function Cabinet({ g }: { g: Game }) {
  const glow = g.soon ? 'none' : `0 0 0 1px ${g.color}55, 0 14px 46px -14px ${g.color}80`
  return (
    <button
      disabled={g.soon}
      className={`group relative overflow-hidden text-left rounded-2xl border transition-all duration-300 ${g.soon ? 'opacity-60 cursor-default' : 'hover:-translate-y-1.5'}`}
      style={{
    ...
    >
      {/* ART / PICTURE AREA — game image goes here later */}
      <div className="relative h-44 grid place-items-center"
        style={{ background: `radial-gradient(120% 120% at 50% 0%, ${g.color}33, transparent 70%)` }}>
        <span className="grid place-items-center w-20 h-20 rounded-2xl"
          style={{ color: g.color, background: `${g.color}1f`, boxShadow: g.soon ? 'none' : `0 0 30px ${g.color}77` }}>
          <span className="[&>svg]:w-9 [&>svg]:h-9">{g.icon}</span>
        </span>
        {g.featured && !g.soon && (
          <span className="absolute top-3 left-3 text-[9px] font-pixel px-2 py-1 rounded bg-neon-amber text-[#2a1a00]">DAILY</span>
        )}
        {g.soon && (
          <span className="absolute top-3 right-3 text-[9px] font-pixel px-2 py-1 rounded bg-white/10 text-white/70">SOON</span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between px-5 py-4 border-t border-white/10">
        <div className="min-w-0">
          ...
          {!g.soon && (
            <div className="text-xs text-white/45 mt-0.5">Best <span className="font-bold tabular-nums" style={{ color: g.color }}>{g.best?.toLocaleString()}</span></div>
          )}
        </div>
        {!g.soon && (
          <span className="text-[10px] font-pixel px-4 py-2.5 rounded-lg text-[#0a0620] transition group-hover:brightness-110 shrink-0"
